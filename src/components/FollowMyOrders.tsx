    ...
    useEffect (() => {
        const getOrder = async () => {
            try {
                ...
                if(bakery !== null) {
                    const response = await api.get(`/order/all-by-user/${bakery.id}`);
        ...
        <>
            <div className="all-orders-container">
                {loadingOrder ? (
                    ...
                    orders.length === 0 ? (
                        <h3>Não existem encomendas para acompanhar.</h3>
                    ) : (
                        ...
                        Object.entries(grouped).map(([day, orders]) => (
                            <div key={day}>
                                ...
                                <div className="orders-group">
                                    {orders.map((order) => (
                                        <OrderShow key={order.id + order.date} order={order} myOrders={true} refreshOrders={() => refreshOrder()} mode="normal"/>
                                    ))}
                                </div>
                            </div>
                        ))
